...
def _resolve_video_resolution_provider(provider_name: str, video_backend: Any | None = None) -> str:
    ...
    if get_provider_fallback(mapped, default="") != "":
        return mapped
    ...
    if isinstance(delegate_name, str):
        ...
        if get_provider_fallback(mapped_delegate, default="") != "":
...
async def execute_video_task(
    ...
    resolved_resolution = await resolve_resolution(
        project,
        registry_provider_id or provider_name,
        model_name or "",
    )
    if resolved_resolution is None:
        resolution_provider = _resolve_video_resolution_provider(
            provider_name,
            getattr(generator, "_video_backend", None),
        )
        resolution = get_provider_fallback(resolution_provider)
    else:
